...
class LatencyTracker:
    ...
    def record_latency(self, latency_ms: float, resource_data: Optional[Dict[str, float]] = None) -> None:
        """
        ...
        if resource_data:
            ...
            self._resource_samples.append(self._capture_current_resources())
    
    def _capture_current_resources(self) -> Dict[str, float]:
        """
    ...
    def calculate_p50_latency(self) -> Optional[float]:
        """
    ...
    def calculate_p95_latency(self) -> Optional[float]:
        """
        Calculate the P95 latency (95th percentile).
    ...
    def calculate_p99_latency(self) -> Optional[float]:
        """
        Calculate the P99 latency (99th percentile).
    ...
    def get_percentile_metrics(self) -> Dict[str, Optional[float]]:
        """
        ...
        """
        return {
            "p50_latency_ms": self.calculate_p50_latency(),
...
class TelemetryManager:
    ...
    @classmethod
    async def get_latency_tracker(cls) -> LatencyTracker:
        """
        Get or create the shared latency tracker instance (thread-safe).
        ...
        async with cls._latency_tracker_lock:
            if cls._latency_tracker is None:
                cls._latency_tracker = LatencyTracker()
            return cls._latency_tracker
    
    @staticmethod
    async def record_request_latency(latency_ms: float, resource_data: Optional[Dict[str, float]] = None) -> None:
        """
        ...
        tracker.record_latency(latency_ms, resource_data)
        
        # Update telemetry file with latest metrics
        await TelemetryManager._update_latency_metrics()
    
    @staticmethod
    async def _update_latency_metrics() -> None:
        """
        Update the telemetry file with current latency and resource metrics.
        """
        tracker = await TelemetryManager.get_latency_tracker()
        
        latency_data = {
            "latency_sample_count": len(tracker.get_latency_samples()),
            "p50_latency_ms": tracker.calculate_p50_latency(),
            "p95_latency_ms": tracker.calculate_p95_latency(),
            "p99_latency_ms": tracker.calculate_p99_latency(),
            "avg_latency_ms": tracker.calculate_avg_latency(),
            "resource_metrics": tracker.get_resource_metrics(),
            "last_latency_update": datetime.utcnow().isoformat() + "Z"
        }
        
        await TelemetryManager.update(latency_data)
...
class LatencyTimer:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.start_time is not None and self.end_time is not None:
            ...
            await TelemetryManager.record_request_latency(latency_ms, self.resource_data)
        return False
